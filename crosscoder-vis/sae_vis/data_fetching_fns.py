...
# from sae_lens import SAE, HookedSAETransformer  # Now handled in mock section
from torch import Tensor
from tqdm.auto import tqdm
from transformer_lens import ActivationCache, HookedTransformer, utils

# Add crosscoder support and mock sae_lens if not available
try:
...
try:
    import sys
    import os
    sys.path.append(os.path.join(os.path.dirname(__file__), '../../../crosscoder'))
    from crosscoder import Crosscoder
    CROSSCODER_AVAILABLE = True
except ImportError:
    CROSSCODER_AVAILABLE = False
    Crosscoder = None
...
def get_crosscoder_feature_data(
    crosscoder,  
    model,
    tokenizer,
    ...
    """
    Generate crosscoder visualization data in the format expected by the dashboard.
    This is a simplified implementation that generates the data directly.
    ...
    Args:
        ...
        n_sequences: Number of sequences per feature
        sequence_length: Length of input sequences
        
    Returns:
        Dictionary containing all visualization data for the features
    """
    if not CROSSCODER_AVAILABLE:
        raise ImportError("Crosscoder not available. Please install crosscoder package.")
    ...
    for i, example in enumerate(dataset):
        if i >= 100:  # Get more examples for better feature activation coverage
            break
    ...
    for feature_idx in feature_indices:
        ...
        # Get crosscoder activations for this feature
        with torch.no_grad():
            # Get model activations
            outputs = model(tokens, output_hidden_states=True)
            hidden_states = outputs.hidden_states[4]  # Layer 4 (0-indexed)
            
            # Get crosscoder feature activations
            # crosscoder.W_dec shape: [ae_dim, n_layers, d_model]
            # hidden_states shape: [batch, seq, d_model]
            # We need to compute activations for this feature across the layers it affects
            
            # Get the decoder weights for this feature: [n_layers, d_model]
            feature_decoder = crosscoder.W_dec[feature_idx]  # [n_layers, d_model]
            
            # Compute dot product with hidden states and take max across layers
            # This is a simplified approximation - real crosscoder is more complex
            layer_activations = torch.einsum('bsh,lh->bsl', hidden_states, feature_decoder)  # [batch, seq, n_layers]
            feature_acts = torch.relu(layer_activations.max(dim=-1)[0])  # [batch, seq]
            
            # Find top activating sequences
            flat_acts = feature_acts.flatten()
            top_values, top_indices = torch.topk(flat_acts, k=min(n_sequences, len(flat_acts)))
            ...
            for i, idx in enumerate(top_indices):
                ...
                # Get context window
                start = max(0, seq_idx - 5)
                end = min(tokens.shape[1], seq_idx + 6)
                context_tokens = tokens[batch_idx, start:end]
                
                # Decode tokens
                token_strings = []
                activations = []
                for j, token_id in enumerate(context_tokens):
                    if token_id.item() in [0, 50256]:  # Skip special tokens
                        continue
                    ...
                    if token_str.strip():
                        ...
                        pos_idx = start + j
                        if pos_idx < feature_acts.shape[1]:
                            act = feature_acts[batch_idx, pos_idx].item()
                        else:
                            act = 0.0
                ...
                if len(token_strings) > 0:
                    sequences.append({
                        ...
                        "max_activation": float(top_values[i])
                    })
            
            # Create feature data structure
            feature_data[str(feature_idx)] = {
                "actsHistogram": {
                    "x": [float(x) for x in torch.linspace(0, feature_acts.max().item(), 50)],
                    "y": [int(c) for c in torch.histc(feature_acts, bins=50).tolist()],
                    "ticks": [0, feature_acts.max().item()/2, feature_acts.max().item()],
                    "title": f"Feature {feature_idx} Activations (density: {(feature_acts > 1e-6).float().mean()*100:.1f}%)"
                },
                "logitsHistogram": {
                    "x": [],  # Placeholder
                    "y": [],
                ...
                "seqMultiGroup": [{
                    "seqGroupData": [{
                        "seqData": [{
                            "tok": tok,
                            "tokID": j,
                            "isBold": j == len(seq["tokens"])//2,  # Middle token is bold
                            "featAct": round(act, 4),
                            "tokenLogit": 0.0,  # Placeholder
                            "tokPosn": f"(0, {j})",
                            "posToks": [],
                            "posVal": [],
                            "negToks": [],
                            "negVal": [],
                            "logitEffect": 0.0,  # Placeholder - could compute actual logit effects
                            "lossEffect": 0.0,   # Placeholder
                            "origProb": 0.0,
                            "newProb": 0.0
                        } for j, (tok, act) in enumerate(zip(seq["tokens"], seq["activations"]))],
                        "dfaSeqData": [],
                        "seqMetadata": {}
                    } for seq in sequences[:10]],  # Limit to 10 sequences
                    "title": f"Top Activating Sequences"
                }]
            }
