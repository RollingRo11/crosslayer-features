...
function setupActsHistogram(featureIdx, componentData, containerId) {
    ...
    var layout = {
        ...
        bargap: 0.01,
        showlegend: false,
        margin: {l: 60, r: 30, b: 40, t: 30, pad: 4},
        paper_bgcolor: 'rgba(0,0,0,0)',
        plot_bgcolor: 'rgba(0,0,0,0)',
        autosize: true,
        height: 250,  // Fixed height
        width: undefined,   // Let it fill container width
    ...
    // Force the plot to fit within its container
...
// ! [4/6] crossLayerTrajectory

function setupCrossLayerTrajectory(featureIdx, componentData, containerId) {
    const plotContainer = d3.select(`#${containerId}`).attr("class", "crossLayerTrajectory");

    // Add title as h4 element to match other sections
    plotContainer.node().parentNode.insertBefore(
        document.createElement('h4'), document.getElementById(containerId)
    ).innerHTML = 'CROSS-LAYER FEATURE TRAJECTORY';

    // Create layout for the cross-layer trajectory plot
    var layout = {
        paper_bgcolor: 'white',
        plot_bgcolor: 'white',
        xaxis: {
            ...
            zerolinecolor: '#eee',
            tickmode: 'array',
            tickvals: componentData.layers,
            ticktext: componentData.layers.map(l => l.toString()),
            range: [-0.5, Math.max(...componentData.layers) + 0.5]
        },
        yaxis: {
            title: componentData.normalize ? 'Feature norm (normalized)' : 'Layer Contribution',
            gridcolor: '#eee',
            zerolinecolor: '#eee',
            range: componentData.normalize ? [-0.05, 1.05] : undefined
        },
        showlegend: false,
        margin: {l: 60, r: 30, b: 50, t: 30, pad: 4},
        autosize: true,
        hovermode: 'x unified',
    ...
    // Create traces for individual token trajectories
    var traces = [];
    
    // Color scale for trajectories (similar to the reference image)
    const numTrajectories = componentData.trajectories.length;
    const colorScale = d3.scaleSequential(d3.interpolateViridis).domain([0, numTrajectories - 1]);
    
    // Show only the mean trajectory for the selected feature (single clean line)
    if (componentData.meanTrajectory && componentData.meanTrajectory.some(val => Math.abs(val) > 1e-6)) {
        traces.push({
            x: componentData.layers,
            y: componentData.meanTrajectory,
            type: 'scatter',
            mode: 'lines+markers',
            line: {
                color: 'rgb(220, 20, 60)',  // Crimson red like in your reference
                width: 3
            },
            marker: {
                color: 'rgb(220, 20, 60)',
            ...
            name: 'Feature Trajectory',
            hovertemplate: 'Layer: %{x}<br>Contribution: %{y:.3f}<extra></extra>',
            showlegend: false
        });
    } else {
        // If no meaningful trajectory, show a flat line at zero
        traces.push({
            x: componentData.layers,
            y: new Array(componentData.layers.length).fill(0),
            type: 'scatter',
            mode: 'lines',
            line: {
            ...
            name: 'No Activity',
            hovertemplate: 'Layer: %{x}<br>No significant contribution<extra></extra>',
            showlegend: false
        });
    }

    // Plot the cross-layer trajectory
    Plotly.newPlot(containerId, traces, layout, {
        responsive: true, 
        displayModeBar: false,
        useResizeHandler: true
...
// ! [5/6] logitsTable & probeLogitsTables
...
function _setupSeqMultiGroupStandard(featureIdx, componentData, containerId) {
                ...
                .each(function({seqData}) {
                    seqData.forEach(tokData => {
                        generateTokHtmlElement({
                            ...
                            hide: tokData["hide"],
                            allSeqData: seqData // Pass entire sequence data for aggregated logit effects
                        });
                    });
                });
    });
    ...
    function generateTokHtmlElement({
        ...
        allSeqData = []                    // entire sequence data for aggregated effects
    }) {
        // Figure out if any token in the sequence is active (this affects the construction of the tooltip)
        let anyTokenActive = allSeqData.some(tokData => 
            Math.abs(tokData.featAct || 0) > 1e-8 || 
            tokData.isBold || 
            (tokData.posToks && tokData.posToks.length > 0) || 
            (tokData.negToks && tokData.negToks.length > 0)
        );
        ...
        // If we're not hiding (because we only generated data for the bolded tokens), then create tooltip & add it on hover
            ...
            // First define the tooltip div (added to the parent element, i.e. it's a sibling of the token span)
            let tooltipHeight = isDfa ? 100 : (anyTokenActive ? 320 : 160);
            let tooltipWidth = isDfa ? 200 : (anyTokenActive ? 360 : 250);
            ...
            if (!isDfa) {
                ...
                // If any token in sequence is active, we add logit table & loss info
                if (anyTokenActive) {
                    ...
                    // Aggregate logit effects from all highlighted tokens in sequence
                    let aggregatedPosToks = {};
                    let aggregatedNegToks = {};
                    
                    // Collect effects from all tokens with any highlighting (background color or bold)
                    allSeqData.forEach(tokData => {
                        // Check if token has highlighting (activation > 1e-8 OR is bold OR has logit effects)
                        const hasActivation = Math.abs(tokData.featAct || 0) > 1e-8;
                        const isBold = tokData.isBold;
                        const hasLogitEffects = (tokData.posToks && tokData.posToks.length > 0) || 
                                              (tokData.negToks && tokData.negToks.length > 0);
                        
                        if (hasActivation || isBold || hasLogitEffects) {
                            // Add positive effects
                            (tokData.posToks || []).forEach((posToken, idx) => {
                                const value = tokData.posVal[idx] || 0;
                                if (aggregatedPosToks[posToken]) {
                                    aggregatedPosToks[posToken] += value;
                                } else {
                                    aggregatedPosToks[posToken] = value;
                                }
                            });
                            ...
                            (tokData.negToks || []).forEach((negToken, idx) => {
                                const value = tokData.negVal[idx] || 0;
                                if (aggregatedNegToks[negToken]) {
                                    aggregatedNegToks[negToken] += value;
                                } else {
                                    aggregatedNegToks[negToken] = value;
                                }
                            });
                    ...
                    // If no aggregated effects, fall back to current token's effects
                    if (Object.keys(aggregatedPosToks).length === 0 && Object.keys(aggregatedNegToks).length === 0) {
                        // Use current token's effects as fallback
                        (posToks || []).forEach((posToken, idx) => {
                            aggregatedPosToks[posToken] = posVal[idx] || 0;
                        });
                        (negToks || []).forEach((negToken, idx) => {
                            aggregatedNegToks[negToken] = negVal[idx] || 0;
                        });
                    }
                    
                    // Sort and take top 10 for each
                    const sortedPosToks = Object.entries(aggregatedPosToks)
                        .sort(([,a], [,b]) => b - a)
                        .slice(0, 10);
                    const sortedNegToks = Object.entries(aggregatedNegToks)
                        .sort(([,a], [,b]) => a - b)
                        .slice(0, 10);
                    ...
                    // Create the positive table, and fill it with aggregated values
                    ...
                    sortedPosToks.forEach(([token, value]) => {
                        posLogitsTable.append("tr").html(`
                            <td class="right-aligned"><code>${token.replace(/ /g, '&nbsp;')}</code></td>
                            <td class="left-aligned">${value > 0 ? '+' : ''}${value.toFixed(3)}</td>
                        `);
                    });
        
                    // Create the negative table, and fill it with aggregated values
                    ...
                    sortedNegToks.forEach(([token, value]) => {
                        negLogitsTable.append("tr").html(`
                            <td class="right-aligned"><code>${token}</code></td>
                            <td class="left-aligned">${value > 0 ? '+' : ''}${value.toFixed(3)}</td>
                        `);
                    });
        
                // If no tokens in sequence are active, we add a message instead
                } else {
                    tableContainer.append("div")
                        .style("font-size", "0.8em")
                        .html("Feature not active in sequence;<br>no predictions were affected.");
                }
            }
...
// Setup function for HTML-based components (Plotly plots)
function setupHtmlComponent(featureIdx, componentData, containerId) {
    const container = d3.select(`#${containerId}`);
    container.html(componentData);
}

// ! [5/10] decoderNorms - Feature Strength Profile (Plot 1)
function setupDecoderNorms(featureIdx, componentData, containerId) {
    const plotContainer = d3.select(`#${containerId}`).attr("class", "plotly-plot");
    
    // Add title
    plotContainer.node().parentNode.insertBefore(
        document.createElement('h4'), document.getElementById(containerId)
    ).innerHTML = 'FEATURE STRENGTH PROFILE: DECODER NORMS ACROSS LAYERS';
    
    // Create traces for each feature
    const traces = componentData.feature_indices.map((featIdx, i) => ({
        x: Array.from({length: componentData.n_layers}, (_, i) => i),
        y: componentData.decoder_norms[featIdx],
        type: 'scatter',
        mode: 'lines+markers',
        name: `Feature ${featIdx}`,
        line: {width: 2},
        marker: {size: 6}
    }));
...
// ! [6/10] activationHeatmap - Activation Heatmap (Plot 2)
function setupActivationHeatmap(featureIdx, componentData, containerId) {
    const plotContainer = d3.select(`#${containerId}`).attr("class", "plotly-plot");
    
    // Add title
    plotContainer.node().parentNode.insertBefore(
        document.createElement('h4'), document.getElementById(containerId)
    ).innerHTML = `ACTIVATION HEATMAP FOR FEATURE ${componentData.feature_idx}`;
    
    const trace = {
        z: componentData.activation_matrix,
        x: componentData.token_strings,
        y: Array.from({length: componentData.n_layers}, (_, i) => `Layer ${i}`),
        type: 'heatmap',
        colorscale: 'Viridis',
        text: componentData.activation_matrix.map(row => 
...
// ! [7/10] aggregatedActivation - Aggregated Activation Profile (Plot 3)
function setupAggregatedActivation(featureIdx, componentData, containerId) {
    const plotContainer = d3.select(`#${containerId}`).attr("class", "plotly-plot");
    
    // Add title
    plotContainer.node().parentNode.insertBefore(
        document.createElement('h4'), document.getElementById(containerId)
    ).innerHTML = `AGGREGATED ACTIVATION PROFILE (n=${componentData.n_samples_processed})`;
    
    // Create traces for each feature
    const traces = componentData.feature_indices.map((featIdx, i) => ({
        x: Array.from({length: componentData.n_layers}, (_, i) => i),
        y: componentData.mean_activations[featIdx],
        type: 'scatter',
        mode: 'lines+markers',
        name: `Feature ${featIdx}`,
        line: {width: 2},
...
// ! [8/10] dla - Direct Logit Attribution (Plot 4)
function setupDLA(featureIdx, componentData, containerId) {
    const plotContainer = d3.select(`#${containerId}`).attr("class", "plotly-plot");
    
    // Add title
    plotContainer.node().parentNode.insertBefore(
        document.createElement('h4'), document.getElementById(containerId)
    ).innerHTML = `DIRECT LOGIT ATTRIBUTION FOR FEATURE ${componentData.feature_idx} ACROSS LAYERS`;
    
    // Calculate subplot layout
    const n_layers = componentData.n_layers;
    const cols = Math.min(4, n_layers);
    const rows = Math.ceil(n_layers / cols);
    
    // Create subplots
    const subplot_titles = Array.from({length: n_layers}, (_, i) => `Layer ${i}`);
    
    // Create traces for each layer
    const traces = [];
    for (let layer_idx = 0; layer_idx < n_layers; layer_idx++) {
        const layer_data = componentData.dla_by_layer[layer_idx];
        
        // Combine top and bottom tokens
        const tokens = [...layer_data.top_tokens, ...layer_data.bottom_tokens];
        const values = [...layer_data.top_values, ...layer_data.bottom_values];
        
        // Create bar chart for this layer
        traces.push({
            x: values,
            y: tokens,
            type: 'bar',
            orientation: 'h',
            marker: {
                color: values.map(v => v > 0 ? 'rgba(0, 128, 0, 0.6)' : 'rgba(255, 0, 0, 0.6)')
            },
            showlegend: false,
            hovertemplate: '%{y}: %{x:.3f}<extra></extra>',
            xaxis: `x${layer_idx > 0 ? layer_idx + 1 : ''}`,
            yaxis: `y${layer_idx > 0 ? layer_idx + 1 : ''}`
        });
    }
    ...
    const layout = {
        ...
        width: undefined,
        responsive: true,
        margin: {l: 80, r: 30, b: 30, t: 30},
        grid: {rows: rows, columns: cols, pattern: 'independent'},
    };
    
    // Add axes configurations
    for (let i = 0; i < n_layers; i++) {
        const row = Math.floor(i / cols);
        const col = i % cols;
        const x_key = i === 0 ? 'xaxis' : `xaxis${i + 1}`;
        const y_key = i === 0 ? 'yaxis' : `yaxis${i + 1}`;
        
        layout[x_key] = {
            title: 'Logit Effect',
            gridcolor: '#eee',
            domain: [col/cols + 0.02, (col+1)/cols - 0.02]
        };
        layout[y_key] = {
            tickfont: {size: 8},
            domain: [1 - (row+1)/rows + 0.02, 1 - row/rows - 0.02]
        };
    }
    
    // Add subplot titles as annotations
    layout.annotations = subplot_titles.map((title, i) => ({
        text: title,
        xref: 'paper',
        yref: 'paper',
        x: (i % cols + 0.5) / cols,
        y: 1 - (Math.floor(i / cols)) / rows,
        xanchor: 'center',
        yanchor: 'bottom',
        showarrow: false,
...
// ! [9/10] featureCorrelation - Feature Correlation Heatmap (Plot 5)
function setupFeatureCorrelation(featureIdx, componentData, containerId) {
    const plotContainer = d3.select(`#${containerId}`).attr("class", "plotly-plot");
    
    // Add title
    plotContainer.node().parentNode.insertBefore(
        document.createElement('h4'), document.getElementById(containerId)
    ).innerHTML = `FEATURE CORRELATION HEATMAP (TOP ${componentData.n_features} FEATURES)`;
    
    const trace = {
        z: componentData.correlation_matrix,
        x: componentData.feature_indices.map(i => `F${i}`),
        y: componentData.feature_indices.map(i => `F${i}`),
        type: 'heatmap',
        colorscale: 'RdBu',
        zmid: 0,
        text: componentData.correlation_matrix.map(row => 
...
const componentMap = {
    ...
    "crossLayerTrajectory": setupCrossLayerTrajectory,
    "seqMultiGroup": setupSeqMultiGroup,
    "prompt": setupSeqMultiGroup,
    // New cross-layer visualizations
    "decoderNorms": setupDecoderNorms,
    "activationHeatmap": setupActivationHeatmap,
    "aggregatedActivation": setupAggregatedActivation,
    "dla": setupDLA,
    "featureCorrelation": setupFeatureCorrelation,
};
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const promptVisMode = Object.keys(PROMPT_DATA).length > 0;
    let allKeys = Object.keys(promptVisMode ? PROMPT_DATA : DATA);
    ...
    // If we have feature densities and this is feature-centric mode, create controls
    if (!promptVisMode && typeof FEATURE_DENSITIES !== 'undefined' && Object.keys(FEATURE_DENSITIES).length > 0) {
        // Create controls container
        const controlsContainer = d3.select('#dropdown-container').append('div')
            .attr('id', 'feature-controls')
        ...
        // Sort controls
        const sortSection = controlsContainer.append('div')
            .style('display', 'flex')
            .style('align-items', 'center')
            .style('gap', '10px');

        sortSection.append('label')
            .text('Sort by density:')
            .style('font-weight', 'bold');

        const sortButtonsDiv = sortSection.append('div')
            .style('display', 'flex')
            .style('gap', '5px');

        const descendingBtn = sortButtonsDiv.append('button')
            .text('High → Low')
            .attr('id', 'sort-desc');

        const ascendingBtn = sortButtonsDiv.append('button')
            .text('Low → High')
            .attr('id', 'sort-asc');
        ...
        filterSection.append('label')
            .text('Filter:')
            .style('font-weight', 'bold');

        const filterBtn = filterSection.append('button')
            .text('Hide Dead Features')
            .attr('id', 'filter-dead');

        // State variables
        let currentSort = 'none';
        let filterDeadFeatures = false;

        // Function to update dropdown based on current settings
        function updateFeatureDropdown() {
            let features = Object.keys(DATA);
            
            // Apply filtering
            if (filterDeadFeatures) {
                features = features.filter(feat => (FEATURE_DENSITIES[feat] || 0) > 0);
            }
            
            // Apply sorting
            if (currentSort === 'asc') {
                features.sort((a, b) => (FEATURE_DENSITIES[a] || 0) - (FEATURE_DENSITIES[b] || 0));
            } else if (currentSort === 'desc') {
                features.sort((a, b) => (FEATURE_DENSITIES[b] || 0) - (FEATURE_DENSITIES[a] || 0));
            }

            // Update the dropdown
            const select = d3.select('#select-0');
            if (!select.empty()) {
                ...
                // Add options with density in the text
                features.forEach(feat => {
                    const density = FEATURE_DENSITIES[feat] || 0;
                    const displayText = `Feature ${feat} (${density.toFixed(3)}%)`;
                ...
                if (features.includes(currentValue)) {
                    select.property('value', currentValue);
                } else if (features.length > 0) {
                    select.property('value', features[0]);
                    setupPage(features[0]);
                }
                ...
                features.forEach(feat => {
                    const density = FEATURE_DENSITIES[feat] || 0;
                    const displayText = `Feature ${feat} (${density.toFixed(3)}%)`;
                    const width = measureTextWidth(displayText, "1em system-ui");
                    if (width > maxWidth) { maxWidth = width; }
                });
                d3.select('#select-0').node().parentNode.style.width = `${maxWidth + 45}px`;
            }

            // Update button styles
            descendingBtn.classed('active-sort', currentSort === 'desc');
            ascendingBtn.classed('active-sort', currentSort === 'asc');
            
            const deadCount = Object.keys(DATA).filter(feat => (FEATURE_DENSITIES[feat] || 0) === 0).length;
            filterBtn.classed('active-filter', filterDeadFeatures)
                .text(filterDeadFeatures ? `Show Dead Features (${deadCount})` : `Hide Dead Features (${deadCount})`);
        }

        // Add event listeners
        descendingBtn.on('click', () => {
            currentSort = currentSort === 'desc' ? 'none' : 'desc';
            updateFeatureDropdown();
        });

        ascendingBtn.on('click', () => {
            currentSort = currentSort === 'asc' ? 'none' : 'asc';
            updateFeatureDropdown();
        });

        filterBtn.on('click', () => {
            filterDeadFeatures = !filterDeadFeatures;
            updateFeatureDropdown();
        });

        // Create the dropdown in this section for feature-centric mode
        const selectDiv = d3.select('#dropdown-container').append('div').attr('class', 'select');
        const selectElem = selectDiv.append('select').attr('id', 'select-0');

        // Add event listener for dropdown changes
        selectElem.on('change', function() { 
            const selectedValue = this.value;
            setupPage(selectedValue);
        });

        // Initialize with density display but no sorting/filtering
        updateFeatureDropdown();
    }
    ...
    if (allKeys.length > 1) {
        ...
        options.forEach((opts, i) => {
            // Skip creating dropdown for feature-centric mode with densities since we handle it above
            if (!promptVisMode && i === 0 && typeof FEATURE_DENSITIES !== 'undefined') {
                // Don't create the dropdown here - it's handled in the controls section
                return;
            }
    ...
    // ! Initial trigger with delay to ensure CSS layout is complete
    // This fixes the common Plotly.js issue with flexbox containers
    setTimeout(() => {
        requestAnimationFrame(() => {
            setupPage(START_KEY);
            
            // Manual resize trigger for all plots after layout settles
            setTimeout(() => {
                resizeAllPlots();
            }, 100);
        });
...
// Function to manually resize all Plotly plots
function resizeAllPlots() {
    // Find all plotly containers and resize them
    const plotlyContainers = document.querySelectorAll('.plotly-hist, .crossLayerTrajectory');
    plotlyContainers.forEach(container => {
        if (container.id && document.getElementById(container.id)) {
            try {
                Plotly.Plots.resize(container.id);
            } catch (e) {
                console.log(`Could not resize plot ${container.id}:`, e);
            }
        }
    });
}