...
def generate_dashboard_html(feature_data: Dict[str, Any], start_feature: int = 0):
    ...
    # Read the JS file
    js_file = Path(__file__).parent / "crosscoder-vis" / "sae_vis" / "init.js"
...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=5, 
                       help='Number of features to process in each batch (default: 5)')
    parser.add_argument('--output', type=str, default='crosscoder_dashboard.html',
                       help='Output filename (default: crosscoder_dashboard.html)')
    
    args = parser.parse_args()
    
    print(f"Generating dashboard for {args.num_features} features, processing in batches of {args.batch_size}")
    ...
    feature_data = {}
    n_features = min(args.num_features, crosscoder.ae_dim)
    
    # Process features in batches
    for batch_start in range(0, n_features, args.batch_size):
        batch_end = min(batch_start + args.batch_size, n_features)
        batch_features = list(range(batch_start, batch_end))
        
        print(f"Processing batch: features {batch_start}-{batch_end-1}")
        
        for feature_idx in tqdm.tqdm(batch_features, desc=f"Batch {batch_start//args.batch_size + 1}"):
            feature_data[str(feature_idx)] = vis_data.create_feature_data(feature_idx)
        
        # Optional: Clear some memory between batches if needed
    ...
    print(f"Generated visualization for {len(feature_data)} features")
