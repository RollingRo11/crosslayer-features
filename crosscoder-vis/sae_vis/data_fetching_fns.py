...
def parse_feature_data(
    ...
    if layout.feature_tables_cfg is not None and feature_out_dir is not None:
        ...
        if layout.feature_tables_cfg.neuron_alignment_table:
            ...
            pct_of_l1: Arr = np.absolute(top3_neurons_aligned.values) / feature_out_l1_norm.float().cpu().numpy()
...
def _get_feature_data(
    ...
    model_A: NNsightWrapper,
    model_B: NNsightWrapper,
    tokens: Int[Tensor, "batch seq"],
    feature_indices: int | list[int],
    cfg: SaeVisConfig,
    progress: list[tqdm] | None = None,
    ...
    Args:
        ...
        model: NNsightWrapper
            The model we'll be using to get the feature activations. It's a wrapping of the base NNsight model.
...
def get_feature_data(
    ...
    model_B_wrapper = NNsightWrapper(model_B, cfg.hook_point)
...
def parse_prompt_data(
    ...
    for seq_pos, seq_key in enumerate([f"{t!r} ({i})" for i, t in enumerate(str_toks)]):
        ...
        feat_acts_nonzero_filter = (feat_acts[seq_pos] > 0).cpu().numpy()
        ...
        prev_feat_acts_nonzero_filter = (feat_acts[seq_pos - 1] > 0).cpu().numpy()
...
def get_prompt_data(
    ...
    model_wrapped = NNsightWrapper(model, cfg.hook_point)
    ...
    # Calculate & store feature activations
    model_acts = torch.stack([act_post, act_post], dim=0)  # Placeholder for dual model
    model_acts = model_acts[:, :, 1:, :]  # drop bos if needed
    x_cent = model_acts
    ...
    feat_acts = F.relu(feat_acts_pre + feature_bias).squeeze(0)  # [seq feats]
