...
class FeatureTablesData:
    ...
    Inputs:
        neuron_alignment...
            ...
            containing which neurons in the transformer the crosscoder feature is most aligned with.

        correlated_neurons...
            The data for the correlated neurons table (each of its 3 cols). In other words, the data
            containing which neurons in the transformer are most correlated with the crosscoder feature.

        correlated_features...
            The data for the correlated features table (each of its 3 cols). In other words, the data
            containing which features in this crosscoder are most correlated with each other.

        correlated_b_features...
            The data for the correlated features table (each of its 3 cols). In other words, the data
            containing which features in crosscoder-B are most correlated with those in the original crosscoder. Note,
            this one might be absent if we're not using a B-crosscoder.
    """
...
@dataclass
class LayerActivationPlotData:
    """Data for plotting feature activation across layers."""
    ...
    ) -> "LayerActivationPlotData":
        """Create LayerActivationPlotData from raw activation data."""
        # Calculate mean activations across all tokens for each layer
        mean_activations = [
    ...
    def data(
        ...
        """Convert to format expected by JavaScript plotting function."""
        return {
            "layers": self.layers,
            "activations": round_2d_list(self.activations),
            "meanActivations": round_1d_list(self.mean_activations),
            "tokenPositions": self.token_positions,
        }
...
class CrossLayerTrajectoryData:
    ...
    def data(
        ...
        return {
            ...
            "layers": self.layers,
            "trajectories": [round_1d_list(traj) for traj in self.trajectories],
            "sequenceLabels": self.sequence_labels,
            "meanTrajectory": round_1d_list(self.mean_trajectory),
            "peakLayer": self.peak_layer,
            "height": cfg.height,
            "normalize": cfg.normalize,
            "showMean": cfg.show_mean,
        }
...
class SequenceData:
    ...
    Data which is specific to certain model or crosscoder classes:
        ...
        if isinstance(cfg, SeqMultiGroupConfig) and cfg.othello:
            ...
            for i in range(len(self.token_ids)):
                ...
                if bold_idx is not None:
                    # For max bolding, find the token with highest activation that meets the threshold
                    if bold_idx == "max":
                        ...
                        if significant_acts:
                            ...
                            # If no tokens meet threshold, use global max as fallback
                            feat_acts_array = np.array(feat_acts) if not isinstance(feat_acts, np.ndarray) else feat_acts
                            max_pos = np.argmax(np.abs(feat_acts_array)).item()
                            is_max_token = (i == max_pos)
                    else:
                        is_max_token = False
        ...
        seqGroupData = [
            seq._get_seq_data(
                ...
                component_specific_kwargs=dict(
                    bold_idx="max",  # Always use max activation for bolding
        ...
        return [
            sequences_group._get_seq_group_data(
                ...
                component_specific_kwargs=dict(
                    ...
                    bold_idx="max",  # Bold the token with highest activation
                ),
            )
...
class CrosscoderVisData:
    ...
    Args:
        ...
        model:              Model that our crosscoder was trained on
        crosscoder:        Used to get the feature activations
    ...
    cfg: CrosscoderVisConfig = field(default_factory=CrosscoderVisConfig)

    model: LanguageModel | None = None
    crosscoder: Crosscoder | None = None
    crosscoder_B: Crosscoder | None = None
    linear_probes: list[
    ...
    def __post_init__(self):
        ...
        if self.vocab_dict is None:
            ...
            # For NNsight models, we need to get the vocab differently
            tokenizer = self.model.tokenizer
            vocab = tokenizer.get_vocab()
            vocab_dict = {v: k for k, v in vocab.items()}
            self.vocab_dict = {
                k: vocab_dict for k in ["embed", "unembed", "probes"]
            }
        self.decode_fn = get_decode_html_safe_fn(self.vocab_dict)  # type: ignore

    def update(self, other: "CrosscoderVisData") -> None:
        """
        Updates CrosscoderVisData with the data from another CrosscoderVisData object. This is useful during the `get_feature_data`
    ...
    def create(
        ...
        Optional args:
            ...
            crosscoder_B: Crosscoder
                Extra crosscoder for computing stuff like top feature correlations between the 2 crosscoders.
            ...
            linear_probes: list[tuple[str, str, Tensor]]
                ...
                probe consists of:
                    label:   either "input" or "output". If "input" then the table will tell us which probe dirs the crosscoder
                             fires strongly on; if "output" then it tells us which dirs the crosscoder writes to strongly.
    ...
    def save_feature_centric_vis(
        ...
        for feat, components_dict in all_feature_data:
            if "actsHistogram" in components_dict:
                # Access the raw histogram data object directly
                hist_obj = components_dict["actsHistogram"]
                if hasattr(hist_obj, 'title'):
                    title = hist_obj.title
                    # Parse density from title like "DENSITY = 1.234%"
                    import re
                    density_match = re.search(r'DENSITY = ([\d.]+)%', title)
                    if density_match:
                        feature_densities[feat] = float(density_match.group(1))
                    else:
        ...
        # Create DATA with all features (no filtering by default)
        DATA = {}
        FEATURE_DENSITIES = {}
        for feat, components_dict in all_feature_data:
...
@dataclass
class CrossLayerDecoderNormsData:
    """Data for decoder norms visualization (Plot 1)."""
...
@dataclass
class CrossLayerAggregatedActivationData:
    """Data for aggregated activation profile (Plot 3)."""
    feature_indices: list[int]
...
@dataclass
class CrossLayerDLAData:
    """Data for direct logit attribution visualization (Plot 4)."""
    feature_idx: int
    ...
    def data(self, **kwargs):
        ...
        return {
            "type": "dla",
            "feature_idx": self.feature_idx,
            "dla_by_layer": self.dla_by_layer,
            "n_layers": self.n_layers,
            "top_k": self.top_k
        }
...
@dataclass
class CrossLayerFeatureCorrelationData:
    """Data for feature correlation heatmap (Plot 5)."""
